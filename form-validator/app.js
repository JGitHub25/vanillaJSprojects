...
email.addEventListener("change", () => {
  ...
  } else if (email.classList.contains("input-field-error")) {
...
password.addEventListener("change", () => {
  ...
  } else if (password.classList.contains("input-field-error")) {
...
password2.addEventListener("change", () => {
  ...
  } else if (password2.classList.contains("input-field-error")) {
...
const checkNsubmit = (e) => {
  ...
  fieldArray.forEach((field) => {
    if (field.value !== "" && !field.classList.contains("input-field-error")) {
      completeForm[
        field.id
      ] = `${field.value} is a valid value - ${field.validity.valid}`;
    } else {
